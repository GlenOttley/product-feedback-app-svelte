...
<main class="container mt-6">
	...
	>
		...
		Go Back</a
	>
	<div class="p-6 pt-11 bg-white rounded-lg tracking-[-0.25px] relative md:py-[52px] md:px-[42px]">
		<img
			src={newFeedbackIcon}
			alt=""
		...
		<form action="?/addProductRequest" method="post" use:enhance={handleAddProductRequest}>
          ...
          {form && Object.keys(form.errors.description).length
			...
			<div class="flex flex-col gap-4 md:flex-row-reverse ">
				<button type="submit" class="button--purple">Add Feedback</button>
				<a href={previousPage} class="button--gray">Cancel</a>
			</div>
		</form>
	</div>
</main>
